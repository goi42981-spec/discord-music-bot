...
export async function addToQueue(interaction, voiceChannel, tracks) {
  ...
  if (!queue.connection) {
    ...
    try { await entersState(queue.connection, VoiceConnectionStatus.Ready, 20_000); } catch (err) {
      queue.connection.destroy(); queues.delete(guildId); throw new Error('Не удалось подключиться к голосовому каналу.');
    }
    queue.connection.on(VoiceConnectionStatus.Disconnected, async () => {
      try { await Promise.race([entersState(queue.connection, VoiceConnectionStatus.Signalling, 5_000), entersState(queue.connection, VoiceConnectionStatus.Connecting, 5_000)]); } catch { deleteQueue(guildId); }
    });
