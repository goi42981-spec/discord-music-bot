...
export async function addToQueue(interaction, voiceChannel, tracks) {
  ...
  if (!queue.connection) {
    ...
    queue.connection.on('stateChange', (oldState, newState) => {
      ...
      if (newState.status === 'connecting' && newState.networking) {
        newState.networking.on('stateChange', (o, n) => {
          console.log(`[network] ${o.code ?? o.constructor?.name} → ${n.code ?? n.constructor?.name}`);
        });
        newState.networking.on('error', (e) => {
          console.error('[network error]', e.message, e.stack);
        });
        newState.networking.on('debug', (msg) => {
          console.log('[net-debug]', msg);
        });
      }
    });
    ...
    try {
      ...
      console.error('[voice] Не вошёл в Ready. Текущее состояние:', queue.connection.state.status);
